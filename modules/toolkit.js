...
ret.checkInfo = function(itemList, data, hasPasswd) {
	for (i in itemList) {
		if (!itemList[i].necessary && (!data[i] || data[i].length == 0)) {
			continue;
		}
		if (itemList[i].type == "text") {
			if (data[i].search(itemList[i].regExp) == -1) {
				return "Invalid " + itemList[i].label;
			}
		}
		else if (itemList[i].type == "radio") {
			if (itemList[i].contents.indexOf(data[i]) == -1) {
				return "Invalid " + itemList[i].label;
			}
		}
	}
...
ret.getFileType = function(fileName) {
